...
> 🔗 **跨章连接**：$L_i = N_i$，面积坐标就是 §1-5 CST 单元的线性形函数 $N_i = \frac{1}{2\Delta_e}(a_i + b_i x + c_i y)$。那里直接用了这个公式但没解释来源，现在知道它就是"到对边的归一化距离"（详见 [§1-5 §5.4.2(B)](1-5-FEM-Formulation.md#542-单元分析element-analysis)）。
...
> 🔗 **与前文的对应**：这就是 §1-5 CST 单元中用到的形函数，推导过程见 [§1-5 第 593-626 行](1-5-FEM-Formulation.md#542-单元分析element-analysis)——设线性位移场，用 Cramer 法则解节点条件，整理得 $N_s = \frac{1}{2A}(a_s + b_s x + c_s y)$。那里直接用了这个公式但没解释来源，现在知道它就是面积坐标 $L_i$，几何意义是"点到对边的归一化距离"。同理，一维的 $\lambda_i = N_i$（§6.2.1），二维的 $L_i = N_i$（本节），三维的体积坐标 $M_i = N_i$，都是同一个思路的推广。

**一维与二维的对应关系**：

| 概念 | 一维 | 二维 |
|------|------|------|
| 自然坐标 | $\lambda_1, \lambda_2$ | $L_1, L_2, L_3$ |
| 形函数 | $N_i = \lambda_i$ | $N_i = L_i$ |
| 几何含义 | 到对端点的归一化距离 | 到对边的归一化距离 |
| 约束 | $\lambda_1 + \lambda_2 = 1$ | $L_1 + L_2 + L_3 = 1$ |
| 独立变量数 | 1 个 | 2 个 |
| 节点数 | 2 个 | 3 个 |
| 单元形状 | 线段 | 三角形 |
| 形函数表达式 | $\frac{x_{i+1}-x}{L}$ | $\frac{1}{2\Delta}(a_i+b_ix+c_iy)$ |
| 节点处值 | $N_i(\text{节点}j) = \delta_{ij}$ | $N_i(\text{节点}j) = \delta_{ij}$ |
| 形函数之和 | $\sum N_i = 1$ | $\sum N_i = 1$ |
...
**算例**：三角形单元节点坐标 $Q_1(0,0)$，$Q_2(4,0)$，$Q_3(0,3)$。求点 $P(1,1)$ 处的形函数值。

**Step 1**：计算面积

$$2\Delta = \begin{vmatrix}1&0&0\\1&4&0\\1&0&3\end{vmatrix} = 12, \quad \Delta = 6$$

**Step 2**：计算 $a_i, b_i, c_i$

$a_1 = 4\times3 - 0\times0 = 12$，$b_1 = 0-3 = -3$，$c_1 = 0-4 = -4$

$a_2 = 0\times0 - 0\times3 = 0$，$b_2 = 3-0 = 3$，$c_2 = 0-0 = 0$

$a_3 = 0\times0 - 4\times0 = 0$，$b_3 = 0-0 = 0$，$c_3 = 4-0 = 4$

**Step 3**：计算形函数值

$N_1 = \frac{1}{12}(12 + (-3)\times1 + (-4)\times1) = \frac{5}{12}$

$N_2 = \frac{1}{12}(0 + 3\times1 + 0\times1) = \frac{3}{12} = \frac{1}{4}$

$N_3 = \frac{1}{12}(0 + 0\times1 + 4\times1) = \frac{4}{12} = \frac{1}{3}$

验证：$N_1+N_2+N_3 = \frac{5}{12}+\frac{3}{12}+\frac{4}{12} = 1$ ✓
...
**算例**：4 节点矩形单元，节点坐标（自然坐标）：$1(-1,-1)$，$2(1,-1)$，$3(1,1)$，$4(-1,1)$。求点 $P(0,0)$（形心）处的形函数值。

**Step 1**：代入公式 $N_i = \frac{1}{4}(1+\xi_i\xi)(1+\eta_i\eta)$

$N_1 = \frac{1}{4}(1+(-1)\times0)(1+(-1)\times0) = \frac{1}{4}$

$N_2 = \frac{1}{4}(1+1\times0)(1+(-1)\times0) = \frac{1}{4}$

$N_3 = \frac{1}{4}(1+1\times0)(1+1\times0) = \frac{1}{4}$

$N_4 = \frac{1}{4}(1+(-1)\times0)(1+1\times0) = \frac{1}{4}$

验证：$N_1+N_2+N_3+N_4 = 1$ ✓

> 形心处四个形函数值相等（都是 1/4），符合直觉——形心到四个顶点的"距离"相等。
