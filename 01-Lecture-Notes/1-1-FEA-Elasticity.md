...
总势能 $\Pi$ 的**一阶变分**为：
$$\delta\Pi = \int_\Omega \delta\boldsymbol{\varepsilon}^T\boldsymbol{\sigma}\,dV - \int_\Omega \delta\mathbf{u}^T\mathbf{f}\,dV - \int_{S_\sigma} \delta\mathbf{u}^T\bar{\mathbf{T}}\,dS$$

令 $\delta\Pi = 0$ 就得到了**虚位移原理**——这是弹性力学 FEM 推导的起点。

### 1.6.3 最小势能原理 vs 最小余能原理

| | 最小势能原理 | 最小余能原理 |
|--|------------|------------|
| 未知量 | 位移 | 应力 |
| 变量类型 | 位移场 | 应力场 |
| 约束 | 满足位移边界条件 | 满足平衡方程 + 力边界条件 |
| 对应单元 | **位移元**（FEM 中最常用）| 应力元 / 杂交元 |

**位移元的下限性**：因为位移场是"假设"的，结构的实际刚度比 FEM 模型更"柔"，所以 $\Pi_{\text{FEM}} \geq \Pi_{\text{exact}}$，从而位移解 $\leq$ 真实位移。
...
应力 $\sigma_{ij}$ 的第 $i$ 个下标表示应力作用面的法线方向，第 $j$ 个下标表示应力的方向。

- $\sigma_{xx}, \sigma_{yy}, \sigma_{zz}$：作用于垂直于 $x,y,z$ 轴的面上的正应力
- $\tau_{xy} = \sigma_{xy}$：作用于垂直于 $x$ 轴的面上、指向 $y$ 方向的剪应力

### 1.7.2 剪应力互等定理

从微元体的力矩平衡 $\sum M_z = 0$ 可得：
$$\tau_{xy} dy\,dz\,dx = \tau_{yx} dx\,dz\,dy \quad\Rightarrow\quad \tau_{xy} = \tau_{yx}$$

同理 $\tau_{yz} = \tau_{zy}$，$\tau_{zx} = \tau_{xz}$。所以应力张量**对称**，9 个分量减为 6 个独立分量。

### 1.7.3 一点处的应力状态

过一点的不同截面上的应力是不同的。通过**应力张量变换公式**可求任意截面上的应力。主应力是特征值问题 $\det(\sigma_{ij} - \sigma\delta_{ij}) = 0$ 的解。

---

## 1.8 弹性常数之间的关系

各向同性线弹性材料只需 2 个独立常数。常见组合：

| 常数对 | 关系 |
|--------|------|
| 弹性模量 $E$ + 泊松比 $\nu$ | **最常见** |
| Lame 常数 $\lambda$ + 剪切模量 $G$ | 张量形式中最简洁 |
| 体积模量 $K$ + 剪切模量 $G$ | 常用于地学 |
...
> $\nu \to 0.5$ 时材料接近不可压缩（橡胶、水），此时 $\lambda \to \infty$，$K \to \infty$。FEM 中处理不可压缩材料需要特殊技术（选择性减缩积分等）。
...
| 术语 | 英文 | 含义 |
|------|------|------|
| 弹性体 | Elastic Body | 施加外力时变形，卸载后恢复原状的物体 |
| 应力 | Stress | 单位面积上的内力 |
| 应变 | Strain | 单位长度的变形量 |
| 位移 | Displacement | 物体内一点的位置变化 |
| 弹性模量 | Young's Modulus $E$ | 衡量材料抵抗拉压变形的能力 |
| 泊松比 | Poisson's Ratio $\nu$ | 横向应变与纵向应变之比的负值 |
| 剪切模量 | Shear Modulus $G$ | 衡量材料抵抗剪切变形的能力 |
| 圣维南原理 | Saint-Venant's Principle | 在距载荷作用区足够远处，等效载荷产生的效果相同 |
...
|------|--------|----------|------|
| 杆单元 (Truss) | 2 | 线性 | 仅轴力 |
| 梁单元 (Beam) | 2 | 三次 Hermite | 弯曲+轴力 |
| CST (Tri3) | 3 | 线性 | 常应变、最简单 |
| LST (Tri6) | 6 | 二次 | 应变线性变化 |
| Q4 (Quad4) | 4 | 双线性 | 规则区域精度好 |
| Q8 (Quad8) | 8 | 二次 Serendipity | 精度高、无内节点 |
| 四面体 (Tet4) | 4 | 线性 | 任意 3D 几何 |
| 六面体 (Hex8) | 8 | 三线性 | 精度最高 |
...
| 年份 | 人物 | 贡献 |
|------|------|------|
| 1678 | Robert Hooke | 发现弹性体的位移与外力成正比（Hooke 定律） |
| 1821 | Claude Navier | 推导出弹性体的平衡微分方程 |
| 1823 | Augustin-Louis Cauchy | 建立线弹性边值问题的完整数学框架 |
| 1855 | Adhémar Saint-Venant | 扭转和弯曲理论（圣维南原理） |
| 1908 | Walter Ritz | 提出基于能量泛函的近似解法 |
| 1943 | Richard Courant | 分片三角形区域近似，预示 FEM 思想 |
| 1956 | Turner, Clough 等 | 矩阵位移法解平面应力问题 |
| **1960** | **Ray Clough** | **首次提出"Finite Element Method"** |
| 1965 | Zienkiewicz, Cheung | 推广到场问题（热传导、渗流等） |

---

## 检查你的理解

1. FEM 为什么要将连续体离散化？离散化的代价是什么？
2. 弹性力学的 6 个基本假设分别是什么？如果小变形假设不成立，FEM 公式需要做什么修改？
3. 几何方程、物理方程、平衡方程分别描述了哪些物理量之间的什么关系？
4. 平面应力和平面应变的 $[D]$ 矩阵有何不同？为什么两者不同？
5. 最小势能原理的物理意义是什么？它与虚位移原理是什么关系？
6. 什么是位移元的下限性？FEM 求出的位移解比真实值大还是小？
