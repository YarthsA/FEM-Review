...
**问题**：一根等截面杆（长度 $L$，截面积 $A$，杨氏模量 $E$），左端固定，右端受集中力 $P$。求位移场 $u(x)$。
...
应变能：$U = \frac{1}{2}\int_0^L EA\left(\frac{du}{dx}\right)^2 dx$

外力势能：$W = -Pu(L)$

总势能：$\Pi = \frac{1}{2}\int_0^L EA\left(\frac{du}{dx}\right)^2 dx - Pu(L)$

**Step 2**：选试探函数

边界条件：$u(0)=0$（固定端），$u(L)$ 自由。

取 $u = a_1 \cdot x$（线性函数，满足 $u(0)=0$）

**Step 3**：代入泛函

$$\frac{du}{dx} = a_1, \quad u(L) = a_1 L$$

$$\Pi = \frac{1}{2}EA \cdot a_1^2 \cdot L - P \cdot a_1 L = \frac{EAL}{2}a_1^2 - PL a_1$$

**Step 4**：求极值

$$\frac{\partial\Pi}{\partial a_1} = EAL \cdot a_1 - PL = 0 \Rightarrow a_1 = \frac{P}{EA}$$
