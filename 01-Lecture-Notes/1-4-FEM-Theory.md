...
> ❌ 易错点：力学中 Ritz 法的试探函数形式是 uᵢ=uᵢ⁰+aᵢₙuᵢₙ，这里 uᵢ⁰ 处理非齐次位移边界（在 S_u 上 uᵢ⁰=ūᵢ），uᵢₙ 在 S_u 上为零。初学者常犯的错误是只用 Σaᵢₙuᵢₙ 而忘了加 uᵢ⁰——这会导致位移边界条件无法满足。非齐次边界条件必须单独处理！
...
考虑均布荷载 q 作用下的梁（两端有弯矩 $M_0, M_L$ 和剪力 $Q_0, Q_L$）。
