...
> 💡 **为什么要区分这两种边界条件？** 这直接影响 Ritz 法的试函数选择：
>
> | | 本质边界条件 | 自然边界条件 |
> |---|---|---|
> | **物理含义** | 位移/转角被约束 | 力/弯矩被约束 |
> | **数学来源** | 变分时 $\delta w=0$ 或 $\delta w'=0$ | 变分时 $\delta w\neq 0$，被迫令括号内=0 |
> | **Ritz 法要求** | **试函数必须满足** | **不要求满足** |
> | **为什么？** | 不满足则 $\delta w\neq 0$，推导失效 | 不满足时变分原理自动调整使其满足 |
>
> **直觉理解**：本质边界条件是"硬约束"——位移被锁死了，试函数必须跟着走；自然边界条件是"软约束"——力的平衡由变分自动保证，试函数可以自由发挥。
>
> **这就是为什么 Ritz 法比 Galerkin 法容易**：Ritz 法只需满足位移边界（本质），Galerkin 法还需满足力边界（自然）。要求越少，试函数越好选。

#### 从边界条件到 Ritz 法：下一步是什么？

推导出平衡方程和边界条件后，Ritz 法的**实际操作**是：

1. **选试函数**：只需满足**本质边界条件**（位移 BC），忽略自然边界条件
2. **代入总势能泛函**：$\Pi = U - W$
3. **对系数求极值**：$\partial\Pi/\partial a_i = 0$
4. **解方程组**：得到近似位移场

> 💡 **关键认识**：我们在 §4.2.5 花了很大力气推导平衡方程和边界条件，但 Ritz 法**并不直接求解它们**！Ritz 法走的是另一条路——最小化能量泛函。推导平衡方程的目的是：
> 1. 证明泛函极值问题与微分方程边值问题**等价**（§4.2.1 的结论）
> 2. 理解哪些边界条件是"硬约束"（本质），哪些是"软约束"（自然）
> 3. 在后续 Galerkin 法中，需要同时满足两种边界条件
